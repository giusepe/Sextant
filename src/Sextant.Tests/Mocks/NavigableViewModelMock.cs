...
namespace Sextant.Tests
{
    ...
    /// </summary>
    internal class NavigableViewModelMock : INavigable, IDisposable
    {
        private readonly string _id;
        private ISubject<Unit> _navigatedTo;
        private Subject<Unit> _navigatingTo;
        private Subject<Unit> _navigatedFrom;
        ...
        {
            return Observable.Return(Unit.Default).Do(_ => _navigatedFrom.OnNext(Unit.Default));
        }

        /// <inheritdoc/>
        public IObservable<Unit> WhenNavigatingTo(INavigationParameter parameter)
        {
            return Observable.Return(Unit.Default).Do(_ => _navigatingTo.OnNext(Unit.Default));
        }
